...
def create_train_val_split(folder_path, dataset_name):
    ...
    # Save to CSV files
    train_df.to_csv(os.path.join(args.output_path, f"{dataset_name}_train.csv"), index=False)
    val_df.to_csv(os.path.join(args.output_path, f"{dataset_name}_val.csv"), index=False)
