...
def save_features_and_labels_all_in_one(feature_extractor, dataloader, save_dir, dataset):
        ...
        with torch.no_grad():
            ...
            for images, label, name in tqdm(dataloader):
                images = images.to(device)
                batch_features = feature_extractor(images)
            ...
            names = torch.cat(names, dim=0)
            labels_np = labels.numpy()

            h5_filename = os.path.join(save_dir, f"all_features.h5")

            with h5py.File(h5_filename, "w") as hf:
                hf.create_dataset("features", data=features.cpu().numpy())
...
def main(args):
    ...
    for checkpoint in sorted_paths:
        ...
        if args.all_in_one:
            save_features_and_labels_all_in_one(feature_extractor, train_dataloader, train_dir, train_dataset)
            save_features_and_labels_all_in_one(feature_extractor, test_dataloader, test_dir, test_dataset)
        else:
...
def create_umap(data, labels, save_dir,labels_names, filename_addon="train"):
    ...
    #replace EGMENTED - Class  with nothing
    label_names = [x.replace("SEGMENTED - Class ", "") for x in label_names]
    cbar=plt.colorbar()
