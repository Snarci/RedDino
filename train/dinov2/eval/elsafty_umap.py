...
def extract_features(feature_extractor, dataloader, dataset):
...
def main(args):
    ...
    for checkpoint in sorted_paths:
        ...
        wandb.log(
            {"umap_test": wandb.Image(umap_test), "umap_train": wandb.Image(umap_train)}, step=step
        )
...
def create_umap(data, labels, save_dir, labels_names, filename_addon="train", point_size=1):
    #remove the first 22 char from the labels_names and labels
    #labels_names = {k[22:]: v for k, v in labels_names.items()}
