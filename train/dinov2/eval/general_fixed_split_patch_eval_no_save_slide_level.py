...
def extract_features(feature_extractor, dataloader, dataset, img_size_patch=224):
    ...
    with torch.no_grad():
        ...
        for images, labels, names in tqdm(dataloader):
            ...
            #interpolate image to a multiple of img_size_patch
            if images.shape[-1] % img_size_patch != 0 or images.shape[-2] % img_size_patch != 0:
                diff_x = img_size_patch - images.shape[-1] % img_size_patch
                diff_y = img_size_patch - images.shape[-2] % img_size_patch
                images = torch.nn.functional.interpolate(images, size=(images.shape[-2] + diff_y, images.shape[-1] + diff_x), mode='bilinear')
            #split image into patches
            #print("images.shape", images.shape)
            patches = rearrange(images, 'b c (h s1) (w s2) -> (b h w) c s1 s2', s1=img_size_patch, s2=img_size_patch)
            #extract features
            #print("patches.shape", patches.shape)
            batch_features = feature_extractor(patches)
            #print("batch_features.shape", batch_features.shape)
            #average the features of the on the batch dimension
            batch_features = batch_features.mean(dim=0)
...
#normaliyation and tensor transformation
def base_transform():
...
def main(args):
    ...
    for checkpoint in sorted_paths:
        ...
        if args.umap:
            ...
            if args.umapv2:
                print("using umap v2")
                umap_train = create_umap_v2(train_data, train_labels, umap_dir, train_dataset.class_to_label)
                umap_test = create_umap_v2(test_data, test_labels, umap_dir, test_dataset.class_to_label, "test")
            else:
                print("using umap v1")
                umap_train = create_umap(train_data, train_labels, umap_dir, train_dataset.class_to_label)
                umap_test = create_umap(test_data, test_labels, umap_dir, test_dataset.class_to_label, "test")
            print("umap done")
...
def create_umap_v2(data, labels, save_dir, labels_names, filename_addon="train"):
    # Create a UMAP model and fit it to your data
    reducer = umap.UMAP() 
    umap_data = reducer.fit_transform(data) 
    ...
    scatter = plt.scatter(umap_data[:, 0], umap_data[:, 1], c=labels, s=args.point_size, cmap=cmap)
    
    # Create legend handles using the same color mapping as the scatter plot
    unique_labels = np.unique(labels)
    legend_handles = [mpatches.Patch(color=cmap(i / len(unique_labels)), label=labels_names[label]) for i, label in enumerate(unique_labels)]
